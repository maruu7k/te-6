...
describe("HTML Testing", () => {
  test("HTML should have the correct DOCTYPE declaration", () => {
    ...
    expect(htmlContent).toContain(expectedDoctype);
  });
  ...
  test("HTML should have a welcome message", () => {
    expect(htmlContent).toContain("<h1>Welcome to My Site</h1>"); // changing the word site as it's in html
  });
  ...
  test("HTML should have the correct lang attribute in the <html> tag", () => {
    ...
    const htmlContentWithLang = htmlContent.replace(
      '<html lang="en">',
      `<html lang="${expectedLang}">`
    );
    expect(htmlContentWithLang).toContain(`lang="${expectedLang}"`);
  });
